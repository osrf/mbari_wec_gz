...
//Declaration  simple_interp::Interp1d DefaultDamping;
...
double FS_HydroDynamics::AddedMass(double omega, int i, int j)
{
	...
	simple_interp::Interp1d LinInterp(xd,yd); //Called infrequently for testing so re-establish here
	...
	double omega, int j)
{
	...
	// Perform the interpolation
	simple_interp::Interp1d LinInterpRe(xd,yRed); //Called infrequently for testing so re-establish here
	simple_interp::Interp1d LinInterpIm(xd,yImd); //Called infrequently for testing so re-establish here
...
///  Interpolates from tabulated WAMIT data, omega must be greater than or equal
///  to zero.
Eigen::Matrix<std::complex<double>,6,1> FS_HydroDynamics::WaveExcitingForceComponents(double omega)
{
