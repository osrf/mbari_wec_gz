...
void FS_HydroDynamics::ReadWAMITData_FD(std::string filenm)
{
  ...
  if (ifile3.is_open()) {
    ...
    auto Mod_Xi = Eigen::VectorXd(n_freqs);
    auto Pha_Xi = Eigen::VectorXd(n_freqs);
    auto Re_Xi = Eigen::VectorXd(n_freqs);
    auto Im_Xi = Eigen::VectorXd(n_freqs);
...
double FS_HydroDynamics::AddedMass(double omega, int i, int j)
{
  ...
  mlinterp::interp(
    ...
    &(yd[0]), &yi,       // Output axis (y)
    &(xd[0]), &xi);      // Input axis (x)
...
double FS_HydroDynamics::Damping(double omega, int i, int j)
{
  ...
  std::vector<double> xd;
  std::vector<double> yd;
  for (int n = 0; n < fd_Y.size(); n++) {
    xd.push_back(this->fd_am_dmp_omega(n));
    yd.push_back(this->fd_Y[n](i, j));
  }

  xd.push_back(1e6);  // Approximately Infinite Frequency...
  yd.push_back(fd_Y_inf_freq(i, j));
...
void FS_HydroDynamics::WaveExcitingForceComponents(
  double * Xi_Re, double * Xi_Im,
  double omega, int j)
{
  if (omega < 0) {
  ...
  for (int n = 0; n < this->fd_Re_Xi[0].size(); n++) {
    xd.push_back(this->fd_ext_omega(n));
    yRed.push_back(this->fd_Re_Xi[j](n));
    yImd.push_back(this->fd_Im_Xi[j](n));
  }
  ...
  mlinterp::interp(
    &nd, ni,          // Number of points
    &(yRed[0]), Xi_Re,      // Output axis (y)
    &(xd[0]), &xi);         // Input axis (x)
  mlinterp::interp(
    &nd, ni,          // Number of points
    &(yImd[0]), Xi_Re,      // Output axis (y)
    &(xd[0]), &xi);         // Input axis (x)
}
...
Eigen::VectorXd FS_HydroDynamics::BuoyancyForce(Eigen::VectorXd x)
{
  ...
  BuoyancyForces(3) = this->m_rho * this->m_grav *
    (this->Vol * this->COB(1) - (this->Vol * this->COB(2) + this->S22) * x(3) + this->Vol * COB(0) *
    x(5));           // Roll
  BuoyancyForces(4) = this->m_rho * this->m_grav *
    (-this->Vol * this->COB(0) - (this->Vol * this->COB(2) + this->S11) * x(4) + this->Vol *
    COB(1) * x(5));  // Pitch
...
Eigen::VectorXd FS_HydroDynamics::ExcitingForce()
{
  ...
  if (_exc_tstep_index == 0) {  // Fill in initial values
    ...
    // Compute next needed wave-elevation
    _eta0(_exc_tstep_index) = _IncWave.eta(0, 0, _exc_tstep_index * m_dt + m_tau_exc(0));
  }
  for (int i = 0; i < 6; i++) {
    // Compute conv integrals, no need to adjust ends in trap rule since integrand there is zero.
    ExctForces(i) = m_L_exc(i).dot(
      _eta0.segment(
        _exc_tstep_index - _n_exc_intpts,
        _n_exc_intpts).reverse());
  }
