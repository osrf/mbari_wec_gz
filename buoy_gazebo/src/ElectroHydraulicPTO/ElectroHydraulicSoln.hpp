...
private:
  const double Pset{2000.0};  // 750;  // psi
  // ~50GPM/600psi ~= .33472 in^3/psi -> From SUN RPECLAN data sheet
  const double QPerP{(50.0*241.0 / 60.0) / 600.0};
  std::vector<double> Prelief{0, Pset, Pset + 600.0};
  std::vector<double> Qrelief{0, 0, QPerP*6000.0};

  // psi
  const std::vector<double> Peff
  ...
  const std::vector<std::vector<double> > eff_v
  {
  ...
  const std::vector<std::vector<double> > eff_m
  {
    // {0.10, 0.10, 0.10, 0.10, 0.10, 0.10, 0.10, 0.10, 0.10, 0.10, 0.10, 0.10, 0.10, 0.10,
    //  0.10, 0.10, 0.10, 0.10, 0.10, 0.10, 0.10, 0.10},  // 0Mpa/0psi
...
public:
  ...
  : Functor<double>(2, 2)
  {
    ...
    this->reliefValve.SetData(Prelief.size(), Prelief.data(), Qrelief.data());

    // Set HydrualicMotor Volumetric Efficiency
    this->hyd_eff_v.SetData(Peff, N, eff_v);
    this->hyd_eff_m.SetData(Peff, N, eff_m);
  }
