...
#define PRESS_RELIEF_SETPOINT 2900.0  //psi
#define RELIEFFLOW_PER_PSI (50.0/600.0)  // ~50GPM/600psi ~= .33472 in^3/psi -> Relief valve flow above setpoint, From SUN RPECLAN data sheet
#define CUBICINCHES_PER_GALLON 231.0
#define SECONDS_PER_MINUTE 60.0 //I checked the clock to be sure. :)
...
public:
  ...
  int operator()(const Eigen::VectorXd & x, Eigen::VectorXd & fvec) const
  {
    ...
    const double rpm = std::min(fabs(x[0U]), Neff.back());  //To bad Splinter doesn't default to max value when out of range...
   ...
   double QQ = this->Q;
    if (x[1U] > PRESS_RELIEF_SETPOINT)    //  Pressure relief is a one wave valve, relieves when lower pressure is higher than upper (resisting extension)
      QQ += (x[1U] - PRESS_RELIEF_SETPOINT ) * RELIEFFLOW_PER_PSI*CUBICINCHES_PER_GALLON/SECONDS_PER_MINUTE;

    fvec[0U] = x[0U] - eff_v * SECONDS_PER_MINUTE * QQ / this->HydMotorDisp;
