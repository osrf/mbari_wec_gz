...
def generate_test_description():
    ...
    eh_windtarget = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(pkg_buoy_tests, 'launch', 'eh_windtarget_select.launch.py'),
        ),
        launch_arguments={
            'manual': 'False'
        }.items(),
...
class EHSolverTest(unittest.TestCase):
    ...
    def test_termination(self, eh_windtarget, proc_info):
        proc_info.assertWaitForShutdown(
            process=find_proc(eh_windtarget),
            timeout=200
        )
...
class BuoyExperimentComparisonAfterShutdown(unittest.TestCase):
    ...
    def test_exit_code(self, eh_windtarget, proc_info):
        launch_testing.asserts.assertExitCodes(
            proc_info,
            [launch_testing.asserts.EXIT_OK],
            find_proc(eh_windtarget)
        )
