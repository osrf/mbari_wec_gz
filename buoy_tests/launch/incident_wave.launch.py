...
def generate_test_description():
    ...
    incident_wavetarget = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(pkg_buoy_tests, 'launch', 'incident_wave_select.launch.py'),
        ),
        launch_arguments={
            'manual': 'False'
        }.items(),
...
class EHSolverTest(unittest.TestCase):
    ...
    def test_termination(self, incident_wavetarget, proc_info):
        proc_info.assertWaitForShutdown(
            process=find_proc(incident_wavetarget),
            timeout=200
        )
...
class BuoyExperimentComparisonAfterShutdown(unittest.TestCase):
    ...
    def test_exit_code(self, incident_wavetarget, proc_info):
        launch_testing.asserts.assertExitCodes(
            proc_info,
            [launch_testing.asserts.EXIT_OK],
            find_proc(incident_wavetarget)
        )
