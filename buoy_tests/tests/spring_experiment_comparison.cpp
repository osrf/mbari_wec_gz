...
TEST(BuoyTests, SpringExperimentComparison)
{
  ...
  std::string inputdata_dirname(ignition::common::joinPaths(buoy_tests_share, "test_inputdata"));

  std::string inputdata_filename;
  std::string inputfilelist("SpringTestInputFileList.txt");
  ...
  if (testdata.is_open()) {
    ...
    FAIL() << "could not open input file " <<
      ignition::common::joinPaths(inputdata_dirname, inputdata_filename) << std::endl;
  }
  ...
  .OnPostUpdate(
    [&iterations, &jointEntity, &ResultsData](
      const ignition::gazebo::UpdateInfo & _info,
  ...
  if (EXP_Data) {  // Plot data for user to decide if it's valid.
    ...
    for (int i = 1; i < 16; i++) {
      ...
      Gnuplot gp;
      gp << "set term X11 title  '" << InputData.names[i] << " vs " << InputData.names[i + 2] <<
        " Comparison'\n";
      gp << "set grid\n";
...
//    }
    ...
    epsilon = 1e-2;
    if (!std::equal(
        InputData.LowerSpringPressure.begin(), InputData.LowerSpringPressure.end(),
    ...
    epsilon = 1e-2;
    if (!std::equal(
        InputData.UpperSpringPressure.begin(), InputData.UpperSpringPressure.end(),
    ...
    epsilon = 1e-2;
    if (!std::equal(
        InputData.LowerSpringVolume.begin(), InputData.LowerSpringVolume.end(),
    ...
    epsilon = 1e-2;
    if (!std::equal(
        InputData.UpperSpringVolume.begin(), InputData.UpperSpringVolume.end(),
